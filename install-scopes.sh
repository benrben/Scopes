  ...
  (
    ...
    if [[ "$need_skills" == "1" && -n "$skill_targets" ]]; then
      ...
      if [[ ! -f "$src_update_skill_sh" ]]; then
        ...
        say_err "  ref:  $ref"
        say_err ""
        say_err "Fix: Rerun with --repo pointing at a repo or local checkout that contains scripts/update-skill.sh"
        say_err "  Example: ./install-scopes.sh --repo https://github.com/benrben/Scopes.git --ref main"
        say_err "  Or local: ./install-scopes.sh --repo /path/to/Scopes --ref main"
        exit 1
      fi
      for d in $skill_targets; do
